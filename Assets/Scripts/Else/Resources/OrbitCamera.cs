...
public class OrbitCamera : MonoBehaviour {
	...
	private Vector2 camMovementInput;

	public CameraMode cameraMode = CameraMode.CONNECTED;
	...
	public void OnCameraRotate(InputAction.CallbackContext ctx)
    {
		...
		camMovementInput = new Vector2(inputVector.y, inputVector.x);
		//Debug.Log("Oncamrotate");
	}
	...
	bool ManualRotation () {
		...
		*/
		...
		if (camMovementInput.x < -e || camMovementInput.x > e || camMovementInput.y < -e || camMovementInput.y > e) {
		//if (input.x < -e || input.x > e || input.y < -e || input.y > e) {
			orbitAngles += rotationSpeed * Time.unscaledDeltaTime * camMovementInput;
